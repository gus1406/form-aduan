...
$sql_update_laporan = mysqli_query( $koneksi, "UPDATE laporan SET judul='$judul', isi='$isi', tanggal='$tanggal', lokasi='$lokasi', id_instansi='$id_instansi', id_kategori='$id_kategori', tanggal_diubah='$tanggal_diubah' WHERE id_laporan='$id_laporan'" );
...
if ( $sql_update_laporan ) {
	...
	header("location:../admin-aduan-edit.php?aduan_id=$id_laporan&update=berhasil&gambar=$upload_gambar");
} else {
	if ( $terupload ) {
		$upload_gambar = "berhasil";
	} else {
	...
	header("location:../admin-aduan-edit.php?aduan_id=$id_laporan&update=gagal&gambar=$upload_gambar");
}
?>